...
- 経験年数要件（「○年以上」「○年程度」等）がある項目は、required_yearsに求人票が要求する年数を、actual_yearsにスキルシートに基づく「その技術・領域そのもの」の実務年数を必ず数値で入れること。無関係・別領域の経験年数（他分野の開発経験、社会人経験全体、フリーランス経験全体など）をactual_yearsに合算しない。算出できない場合はactual_years=0とする。年数要件がない項目はrequired_years/actual_yearsともにnullでよい。
  actual_yearsの算出は当てずっぽうにせず、次の手順で機械的に行うこと: (1) スキルシート内でその技術・領域に言及している案件・期間を全て列挙する、(2) 各案件の開始月〜終了月を特定する（不明瞭な場合は記載範囲内で保守的・少なめに見積もる）、(3) 複数の案件期間が重なっている場合は重複部分を二重に数えず、期間を統合してから合計月数を数える、(4) 合計月数を12で割って年数にする。同じスキルシートに対しては毎回同じ計算結果になるようにすること。
...
- 働き方の希望条件は、項目ごとに求人票の記載内容と照らし合わせてmatchesを判定する。求人票に該当する記載がなければreasonに「求人票に記載なし」と明記し、matchesはfalseにする。「どちらでも可」など希望に幅がある項目は、求人票がその範囲に収まっていればmatchesをtrueにする。
  reasonとmatches/meetsの結論が矛盾してはならない。reasonの中で「希望範囲に収まる」「条件を満たす」等、条件を満たす旨を述べておきながらmatches/meetsをfalse・×にする（またはその逆）ような自己矛盾を起こさないこと。
- 懸念点(concerns)には、経験年数不足・スキルのブランク・稼働条件（単価等）のミスマッチに加え、働き方の希望条件との重大な不一致（例: フルリモート希望なのに出社必須）があれば具体的に挙げる。
  単価については、work_style_fitの単価項目でmatches=trueと判定した（＝応募者が登録した希望単価レンジを満たす）にもかかわらず、「経験・スキル水準に対して相場的に低い」といった、応募者が登録していない独自の相場観を根拠に懸念点として指摘したりfit_scoreを下げたりしてはならない。単価が懸念点になり得るのは、あくまでmatches=falseの場合（応募者自身が登録した希望単価レンジを満たさない場合）のみである。
