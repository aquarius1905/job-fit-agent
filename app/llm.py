...
- 経験年数要件（「○年以上」「○年程度」等）は厳密に判定する。求人票が特定の技術・領域（例: Web開発、特定言語、特定ポジション）の実務経験年数を指定している場合、スキルシートに記載された「その技術・領域そのもの」の実務年数のみで判定し、無関係・別領域の経験年数（他分野の開発経験、社会人経験全体、フリーランス経験全体など）を合算して充足したことにしない。対象の技術・領域の実務年数が求人票の年数を明確に満たす場合のみmeets="○"とする。年数がやや不足するが近い実務経験がある場合はmeets="△"とし、reasonに実際の年数（例: 実務◯年、求人要件は◯年）を明記する。年数が大きく不足する、または算出できない場合はmeets="×"とする。経験の幅広さや関連スキルの多さで年数不足を相殺しない。
