...
- **判定履歴** — 過去に判定した案件を一覧で振り返れる。日付順/スコア順の並び替え、ページネーションに対応
- **選考結果の記録** — 履歴の各案件に「書類選考で不採用」「商談で不採用」「採用」を後から記録できる。判定スコアと実際の選考結果を突き合わせて、判定精度そのものを検証する材料になる
...
storage / parsing / ルーティング（ページネーション・ソート・Ajax分岐等）と、経験年数の閾値判定（`_enforce_experience_thresholds`）のような判定ロジックの純粋関数をカバー。`llm.evaluate`はモックしており、Claude APIは呼び出さない。

判定精度そのもの（プロンプトの良し悪し）は別枠のevalテストで検証する。実際にClaude APIを呼び出すためコストと非決定性があり、`pytest`単体では実行されない。
...
- 判定精度のevalは実際に発生したバグの再発防止を目的とした少数のケースのみ。網羅的なテストセットではない
- 求人票URLからの自動取得、案件履歴の重複判定などは構想段階
- `/history`の推定適正単価は、単価が読み取れる求人票がまだ少ないため表示条件（`rate.available`）を満たさず現状非表示。履歴が貯まって条件を満たせば自動的に表示されるようになるため、追加対応は不要
