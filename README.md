...
storage / parsing / ルーティング（ページネーション・ソート・Ajax分岐等）と、経験年数の閾値判定（`_enforce_experience_thresholds`）のような判定ロジックの純粋関数をカバー。Claude API自体を呼び出す判定精度そのもの（プロンプトの良し悪し）はここではテストしておらず、`llm.evaluate`はモックしている。

## 現状の制約・今後

- 個人利用を前提にしたシングルユーザー構成（ストレージがグローバルな単一ファイル）。複数人での利用は現状想定していない
- 自動テストはルーティング・判定ロジックのみ。プロンプト自体の判定精度を継続的に検証する仕組み（eval）はまだない
